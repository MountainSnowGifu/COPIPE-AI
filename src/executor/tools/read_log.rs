...
use crate::executor::{ToolResult, ALLOWED_LOGS, LOG_DIR};
...
pub fn handle(ctx: &ToolContext<'_>, filename: &str) -> ToolResult {
    ...
    let output = if !ALLOWED_LOGS.contains(&filename) {
        format!("ERROR: 不正なログ名 '{filename}'。使用可能: {}", ALLOWED_LOGS.join(", "))
        ...
        if log_path.symlink_metadata().map(|m| m.file_type().is_symlink()).unwrap_or(false) {
            ...
            match std::fs::read(&log_path) {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => "(ログファイルが存在しません)".to_string(),
