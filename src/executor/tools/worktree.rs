...
pub async fn enter(root: &Path) -> ToolResult {
    ...
    // git worktree add
    match git(&["worktree", "add", wt_path.to_str().unwrap_or(""), "-b", &branch], root).await {
        Err(e) => return ToolResult::new("EnterWorktree", format!("ERROR: worktree 作成失敗: {e}")),
        Ok(_) => {}
    }
    ...
    if save_state(root, &state).is_err() {
        ...
        let _ = git(&["worktree", "remove", "--force", wt_path.to_str().unwrap_or("")], root).await;
...
pub async fn exit(root: &Path, action: &str, commit_message: &Option<String>) -> ToolResult {
    ...
    let result = match action {
        ...
        other => Err(format!("不正なアクション '{other}'。'merge' または 'discard' を指定してください。")),
    };
...
async fn do_merge(
    ...
    // worktree 内の変更をコミット
    let staged = git(&["status", "--porcelain"], wt_path).await.unwrap_or_default();
    if !staged.is_empty() {
    ...
    git(&["merge", "--squash", branch], root).await
        .map_err(|e| format!("squash merge 失敗: {e}"))?;

    // コミット
    let msg = commit_message.as_deref().unwrap_or("copipe-ai: apply worktree changes");
    git(&["commit", "-m", msg], root).await
        .map_err(|e| format!("コミット失敗: {e}"))?;

    Ok(format!("変更をメインブランチにマージしました（コミット: '{msg}'）"))
}
