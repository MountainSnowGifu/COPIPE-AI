...
pub(super) async fn write_browser_log(root: &std::path::Path, reason: &str, session: &CopilotSession) {
    ...
    let mut f = match std::fs::OpenOptions::new().create(true).append(true).open(&log_path) {
        Ok(f) => f,
        Err(_) => return,
    };
...
pub(super) fn write_ai_log(
    ...
    for cmd in commands {
        let line = serde_json::to_string(cmd).unwrap_or_else(|_| "{\"error\":\"serialize failed\"}".to_string());
        entry.push_str(&line);
        entry.push('\n');
    }
